...
// 
/* 
 * $Id: /drqueue/remote/trunk/drqman/drqm_jobs_pixie.c 2324 2005-05-22T17:22:28.853709Z oliverhotz	$ 
 */
...
GtkWidget *dnj_koj_frame_pixie (struct drqm_jobs_info *info)
{
	...
	/* Frame */
	frame = gtk_frame_new ("Pixie job information");
	...
	label = gtk_label_new ("RIB file:");
	gtk_box_pack_start (GTK_BOX(hbox),label,TRUE,TRUE,2);
	hbox2 = gtk_hbox_new (FALSE,0);
	gtk_box_pack_start (GTK_BOX(hbox),hbox2,TRUE,TRUE,0);
	entry = gtk_entry_new_with_max_length (BUFFERLEN-1);
	info->dnj.koji_pixie.escene = entry;
	gtk_tooltips_set_tip(tooltips,entry,"File name of the pixie RIB file that should be rendered",NULL);
	gtk_box_pack_start (GTK_BOX(hbox2),entry,TRUE,TRUE,2);
	button = gtk_button_new_with_label ("Search");
	gtk_tooltips_set_tip(tooltips,button,"File selector for the pixie RIB file",NULL);
	gtk_box_pack_start (GTK_BOX(hbox2),button,FALSE,FALSE,2);
	g_signal_connect (G_OBJECT(button),"clicked",
										G_CALLBACK(dnj_koj_frame_pixie_scene_search),&info->dnj.koji_pixie);
	...
	info->dnj.koji_pixie.eviewcmd = entry;
	gtk_entry_set_text(GTK_ENTRY(entry),KOJ_PIXIE_DFLT_VIEWCMD);
	...
	info->dnj.koji_pixie.escript = entry;
	gtk_entry_set_text (GTK_ENTRY(entry),pixiesg_default_script_path());
	gtk_box_pack_start (GTK_BOX(hbox2),entry,TRUE,TRUE,2);
	button = gtk_button_new_with_label ("Search");
	gtk_tooltips_set_tip(tooltips,button,"File selector for the script directory",NULL);
	gtk_box_pack_start (GTK_BOX(hbox2),button,FALSE,FALSE,2);
	g_signal_connect (G_OBJECT(button),"clicked",
										G_CALLBACK(dnj_koj_frame_pixie_script_search),&info->dnj.koji_pixie);
	...
	case KOJ_PIXIE:
		g_signal_connect (G_OBJECT(button),"clicked",
											G_CALLBACK(dnj_koj_frame_pixie_bcreate_pressed),&info->dnj);
		break;
	default:
		fprintf (stderr,"What ?!\n");
		break;
...
GtkWidget *jdd_koj_pixie_widgets (struct drqm_jobs_info *info)
{
	...
	char *labels[] = { "RIB File:", info->jdd.job.koji.pixie.scene,
				 "View command:", info->jdd.job.koji.pixie.viewcmd,
				 NULL };
	char **cur;
...
static void dnj_koj_frame_pixie_scene_search (GtkWidget *button, struct drqmj_koji_pixie *info)
{
	...
	gtk_signal_connect (GTK_OBJECT (GTK_FILE_SELECTION(dialog)->ok_button),
					"clicked", GTK_SIGNAL_FUNC (dnj_koj_frame_pixie_scene_set), info);
