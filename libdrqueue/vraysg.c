...
char *vraysg_create (struct vraysgi *info) {
  /* This function creates the vray render script based on the information given */
  /* Returns a pointer to a string containing the path of the just created file */
  /* Returns NULL on failure and sets drerrno */
  FILE *f;
  FILE *etc_vray_sg;   /* The vray script generator configuration file */
  int fd_etc_vray_sg,fd_f;
  static char filename[BUFFERLEN];
  char fn_etc_vray_sg[BUFFERLEN]; /* File name pointing to DRQUEUE_ETC/vray.sg */
  char buf[BUFFERLEN];
  int size;
  ...
  // prepare script file
  snprintf(fn_etc_vray_sg,BUFFERLEN-1,"%s/vray.sg",getenv("DRQUEUE_ETC"));

  fflush (f);

  if ((etc_vray_sg = fopen (fn_etc_vray_sg,"r")) == NULL) {
    fprintf(f,"\necho -------------------------------------------------\n");
    fprintf(f,"echo ATTENTION ! There was a problem opening: %s\n",fn_etc_vray_sg);
    fprintf(f,"echo So the default configuration will be used\n");
    fprintf(f,"echo -------------------------------------------------\n");
    fprintf(f,"\n\n");
    fprintf(f,"vray -scenefile=$SCENE \n\n");
  } else {
    fd_etc_vray_sg = fileno (etc_vray_sg);
    fd_f = fileno (f);
    while ((size = read (fd_etc_vray_sg,buf,BUFFERLEN)) != 0) {
      write (fd_f,buf,size);
    }
    fclose(etc_vray_sg);
  }
